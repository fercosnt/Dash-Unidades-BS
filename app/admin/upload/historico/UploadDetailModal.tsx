          ...
          <div className="border border-slate-200 rounded-lg overflow-hidden">
            <table className="min-w-full text-sm">
              <thead className="bg-slate-50">
                <tr>
                  <th className="px-3 py-2 text-left font-medium text-slate-700">Paciente</th>
                  ...
                  {tipo === "orcamentos_fechados" && (
                    <th className="px-3 py-2 text-left font-medium text-slate-700">Data fechamento</th>
                  )}
                  {tipo === "tratamentos_executados" && (
                    <>
                      <th className="px-3 py-2 text-left font-medium text-slate-700">Procedimento</th>
                      <th className="px-3 py-2 text-right font-medium text-slate-700">Qtd</th>
                      <th className="px-3 py-2 text-left font-medium text-slate-700">Data execução</th>
                    </>
                  )}
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-200">
                {registros.map((r) => (
                  <tr key={r.id}>
                    <td className="px-3 py-2 text-slate-900">{r.paciente_nome ?? "—"}</td>
                    {(tipo === "orcamentos_fechados" || tipo === "orcamentos_abertos") && (
                    ...
                    {tipo === "orcamentos_fechados" && (
                      <td className="px-3 py-2 text-slate-600">{r.data_fechamento ?? "—"}</td>
                    )}
                    {tipo === "tratamentos_executados" && (
                      <>
                        <td className="px-3 py-2 text-slate-700">{r.procedimento_nome ?? "—"}</td>
                        <td className="px-3 py-2 text-right text-slate-700">{r.quantidade ?? "—"}</td>
                        <td className="px-3 py-2 text-slate-600">{r.data_execucao ?? "—"}</td>
                      </>
                    )}
                  </tr>
                ))}
