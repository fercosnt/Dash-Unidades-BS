  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h2 className="text-xl font-bold text-[#0A2463]">Procedimentos</h2>
          ...
          <div className="flex rounded-lg border border-slate-200 bg-white p-1">
            {(["todos", "ativo", "inativo"] as const).map((f) => (
              <Link
                ...
                className={`rounded-md px-3 py-1.5 text-sm font-medium ${statusFilter === f ? "bg-[#0A2463] text-white" : "text-slate-600 hover:bg-slate-100"}`}
              >
        ...
        <table className="min-w-full divide-y divide-slate-200">
          <thead>
            <tr className="bg-slate-50">
              ...
              <th className="px-4 py-3 text-left text-xs font-medium uppercase text-slate-600">Código Clinicorp</th>
              <th className="px-4 py-3 text-right text-xs font-medium uppercase text-slate-600">Custo fixo</th>
              <th className="px-4 py-3 text-left text-xs font-medium uppercase text-slate-600">Categoria</th>
          ...
          <tbody className="divide-y divide-slate-200">
            {procedimentos.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-slate-500">Nenhum procedimento encontrado.</td>
              </tr>
            ) : (
              procedimentos.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 text-sm font-medium text-slate-900">{p.nome}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{p.codigo_clinicorp ?? "—"}</td>
                  <td className="px-4 py-3 text-right text-sm text-slate-600">{formatMoney(p.custo_fixo)}</td>
                  <td className="px-4 py-3 text-sm text-slate-600">{p.categoria ?? "—"}</td>
                  <td className="px-4 py-3 text-center">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${p.ativo ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-600"}`}>
                      {p.ativo ? "Ativo" : "Inativo"}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <button type="button" onClick={() => p.ativo ? openConfirm("desativar", p) : handleAtivar(p)} className="text-slate-600 hover:text-slate-900 mr-2 text-sm">
                      {p.ativo ? "Desativar" : "Ativar"}
                    </button>
                    <button type="button" onClick={() => openEdit(p)} className="text-[#0A2463] hover:underline text-sm font-medium mr-2">Editar</button>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            <h3 className="text-lg font-semibold text-slate-900 mb-4">{editing ? "Editar procedimento" : "Novo procedimento"}</h3>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Código Clinicorp</label>
                <input type="text" value={form.codigo_clinicorp} onChange={(e) => setForm((f) => ({ ...f, codigo_clinicorp: e.target.value }))} className="w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 focus:border-[#0A2463] focus:ring-1 focus:ring-[#0A2463]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Custo fixo (R$) *</label>
                <input type="number" min={0} step={0.01} value={form.custo_fixo} onChange={(e) => setForm((f) => ({ ...f, custo_fixo: e.target.value }))} className="w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 focus:border-[#0A2463] focus:ring-1 focus:ring-[#0A2463]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Categoria</label>
                <input type="text" value={form.categoria} onChange={(e) => setForm((f) => ({ ...f, categoria: e.target.value }))} className="w-full rounded-md border border-slate-300 px-3 py-2 text-slate-900 focus:border-[#0A2463] focus:ring-1 focus:ring-[#0A2463]" list="categorias-list" />
      ...
      {confirmModal?.tipo === "desativar" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            <h3 className="text-lg font-semibold text-slate-900 mb-2">Desativar procedimento</h3>
      ...
      {confirmModal?.tipo === "excluir" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
            <h3 className="text-lg font-semibold text-slate-900 mb-2">Excluir procedimento</h3>
