    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link
          ...
          className="text-sm font-medium text-[#0A2463] hover:underline"
        >
          ← Voltar ao dashboard
        </Link>
      </div>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold text-[#0A2463]">{clinica.nome}</h1>
            ...
            <span
              className={`inline-block mt-2 px-2 py-0.5 rounded text-xs font-medium ${
                clinica.ativo ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-600"
              }`}
            >
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            <KpiCard label="Faturamento bruto" value={resumo?.faturamentoBruto ?? 0} format="currency" />
            <KpiCard label="Custos totais" value={custosTotais} format="currency" />
            <KpiCard label="Valor líquido" value={resumo?.valorLiquido ?? 0} format="currency" />
            <KpiCard label="Parte Beauty Smile (60%)" value={resumo?.valorBeautySmile ?? 0} format="currency" />
            <KpiCard label="Parte clínica (40%)" value={resumo?.valorClinica ?? 0} format="currency" />
            <KpiCard label="Inadimplência" value={resumo?.totalInadimplente ?? 0} format="currency" />
          </div>
          ...
          {tab === 0 && (
            <div className="rounded-lg border border-slate-200 bg-white overflow-hidden">
              <div className="px-4 py-3 bg-slate-50 border-b flex flex-wrap items-center gap-3">
                ...
                <table className="min-w-full divide-y divide-slate-200 text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left font-medium text-slate-700">Data fechamento</th>
                      <th className="px-4 py-2 text-left font-medium text-slate-700">Indicação</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 bg-white">
                    {orcamentosFechados.length === 0 ? (
                      ...
                      orcamentosFechados.map((row) => (
                        <tr key={row.id}>
                          <td className="px-4 py-2 font-medium text-slate-900">{row.pacienteNome}</td>
                ...
                <table className="min-w-full divide-y divide-slate-200 text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-right font-medium text-slate-700">Valor total</th>
                      <th className="px-4 py-2 text-left font-medium text-slate-700">Status</th>
                      <th className="px-4 py-2 text-left font-medium text-slate-700">Data criação</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 bg-white">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200 text-sm">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="px-4 py-2 text-left font-medium text-slate-700">Paciente</th>
                      <th className="px-4 py-2 text-left font-medium text-slate-700">Procedimento</th>
                      <th className="px-4 py-2 text-right font-medium text-slate-700">Qtd</th>
                      <th className="px-4 py-2 text-left font-medium text-slate-700">Data execução</th>
                      <th className="px-4 py-2 text-right font-medium text-slate-700">Custo (R$)</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 bg-white">
            ...
            <div className="rounded-lg border border-slate-200 bg-white p-4 space-y-4">
              ...
              <div className="border-t border-slate-200 pt-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between border-t border-slate-200 pt-2 font-medium">
                  ...
                  <span className="text-[#0A2463]">{formatCurrency(resumo?.valorLiquido ?? 0)}</span>
                </div>
                <div className="flex justify-between text-slate-600">
