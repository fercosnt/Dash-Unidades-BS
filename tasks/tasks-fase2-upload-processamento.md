...
- [ ] 1.0 Implementar utilitarios de parse e transformacao de planilhas XLSX
  - [ ] 1.1 Instalar dependencia SheetJS: `npm install xlsx`
  - [ ] 1.2 Criar arquivo `src/types/upload.types.ts` com interfaces TypeScript para: dados brutos da planilha (RawOrcamentoRow, RawTratamentoRow), dados transformados (TransformedOrcamentoFechado, TransformedOrcamentoAberto, TransformedTratamento), e payload de upload (UploadPayload com clinica_id, mes_referencia, tipo, registros)
  - [ ] 1.3 Criar `src/lib/utils/formatting.ts` com funcoes utilitarias:
  ...
  - [ ] 1.4 Criar `src/lib/utils/formatting.test.ts` com testes unitarios para todas as funcoes de formatting (incluindo edge cases: valores vazios, formatos inesperados, nomes sem parenteses)
  - [ ] 1.5 Criar `src/lib/utils/xlsx-parser.ts` com funcoes:
  ...
  - [ ] 1.6 Criar `src/lib/utils/xlsx-parser.test.ts` com testes unitarios para funcoes de parse (mock de dados simulando planilha Clinicorp com e sem linha totalizadora)
  - [ ] 1.7 Criar `src/lib/utils/xlsx-transforms.ts` com funcoes de transformacao:
  ...
  - [ ] 1.8 Criar `src/lib/utils/xlsx-transforms.test.ts` com testes unitarios para transformacoes (testar: split por "+", separacao fechados/abertos por status, limpeza de nome, conversao monetaria, deteccao de indicacao, registro com multiplos procedimentos)

- [ ] 2.0 Implementar tela de Upload com formulario, preview e confirmacao
  - [ ] 2.1 Criar componente `src/components/shared/ClinicaSelect.tsx` — dropdown de clinicas ativas buscando de `clinicas_parceiras` onde `ativa = true`, usando Supabase browser client (verificar se ja existe da Fase 1 e reutilizar)
  - [ ] 2.2 Criar componente `src/components/shared/MonthPicker.tsx` — seletor de mes/ano que retorna o primeiro dia do mes selecionado (ex: "2026-03-01"), formato visual "Marco/2026"
  - [ ] 2.3 Criar componente `src/components/upload/UploadForm.tsx` com os campos:
  ...
  - [ ] 2.4 Implementar handler de selecao de arquivo no UploadForm: ao selecionar arquivo, chamar `parseXLSXFile` + `removeHeaderRow` + `removeTotalizationRow` + `parseToObjects`, e em seguida aplicar transformacao correspondente ao tipo selecionado (`transformOrcamentos` ou `transformTratamentos`). Armazenar resultado em state
  - [ ] 2.5 Criar componente `src/components/upload/PreviewTable.tsx` que recebe os dados transformados e exibe:
  ...
  - [ ] 2.6 Implementar verificacao de duplicidade antes de confirmar:
  ...
  - [ ] 2.7 Criar componente `src/components/upload/UploadProgress.tsx` com estados visuais:
  ...
  - [ ] 2.8 Criar pagina `src/app/(admin)/upload/page.tsx` compondo todos os componentes em fluxo multi-step:
  ...
  - [ ] 2.9 Ao confirmar upload, enviar dados transformados para API route POST `/api/upload`: montar payload com `clinica_id`, `mes_referencia`, `tipo`, `registros` (array de dados transformados), e flag `substituir` se duplicidade confirmada. Tratar resposta de sucesso/erro e atualizar UI

- [ ] 3.0 Implementar API Route e integracao com n8n para processamento server-side
  - [ ] 3.1 Adicionar variaveis de ambiente em `.env.local`: `N8N_WEBHOOK_URL` (URL do webhook n8n, server-side only) e `N8N_WEBHOOK_SECRET` (secret compartilhado para autenticacao do webhook)
  - [ ] 3.2 Criar API route `src/app/api/upload/route.ts` com handler POST que:
  ...
  - [ ] 3.3 Na mesma API route, apos insercao dos registros, fazer chamada POST ao webhook n8n:
  ...
  - [ ] 3.4 Criar workflow n8n "Upload Processing" (Workflow 1):
  ...
  - [ ] 3.5 No workflow n8n, implementar match de procedimentos (somente para tipo `tratamentos_executados`):
  ...
  - [ ] 3.6 No workflow n8n, apos processamento, atualizar `upload_batches`:
  ...
  - [ ] 3.7 Implementar endpoint ou logica de polling no frontend para verificar status do batch apos envio: consultar `upload_batches` por id a cada 3 segundos ate status mudar de "processando" para "concluido" ou "erro", e atualizar UploadProgress correspondente

- [ ] 4.0 Implementar tela de Historico de Uploads
  - [ ] 4.1 Criar componente `src/components/upload/UploadHistoryTable.tsx` com tabela exibindo colunas:
  ...
  - [ ] 4.2 Implementar filtros no UploadHistoryTable:
  ...
  - [ ] 4.3 Criar componente `src/components/upload/UploadDetailModal.tsx` — ao clicar em uma linha do historico, abrir modal/drawer mostrando:
  ...
  - [ ] 4.4 Criar pagina `src/app/(admin)/upload/historico/page.tsx` usando template ListWithFilters do design system, compondo UploadHistoryTable + UploadDetailModal. Ordenar por data decrescente (uploads mais recentes primeiro)

- [ ] 5.0 Implementar tela de Revisao de Match de Procedimentos
  - [ ] 5.1 Criar pagina `src/app/(admin)/upload/revisao/page.tsx` que consulta `tratamentos_executados` onde `procedimento_id IS NULL`, exibindo:
    - Nome do procedimento (da planilha — campo `procedimento_nome`)
    - Nome do paciente
    - Data de execucao
    - Clinica (via join)
    - Batch de origem (link para detalhe do upload)
  - [ ] 5.2 Criar componente `src/components/upload/ProcedureMatchReview.tsx` com funcionalidade de match manual:
  ...
  - [ ] 5.3 Criar componente `src/components/upload/CreateProcedureModal.tsx` — modal de criacao rapida de procedimento:
  ...
  - [ ] 5.4 Adicionar badge no menu lateral (Sidebar do layout admin) indicando quantidade de procedimentos pendentes de revisao:
...
- [ ] 6.0 Implementar indicador de progresso de upload mensal
  - [ ] 6.1 Criar componente `src/components/upload/MonthlyUploadStatus.tsx` que para cada clinica ativa e mes selecionado:
  ...
  - [ ] 6.2 Adicionar badge visual "2/2 completo" com icone de sucesso quando ambos os uploads de uma clinica/mes estiverem com status "concluido"
  - [ ] 6.3 Integrar MonthlyUploadStatus na tela de upload (`/(admin)/upload/page.tsx`), exibindo o painel de progresso abaixo ou ao lado do formulario de upload para que o admin veja rapidamente o que falta importar
